...
def get_plan(
    ...
    maintenance_plan_generator over the TOP-N impact-ranked assets (not the
    full asset list -- a "prioritized plan" that includes every asset,
    including ones 300+ days from failure, isn't actually prioritized).
    Returned as-is: {"plan": [...], "summary": str}.
    """
    ranked = rank_assets_by_impact(get_all_assets())
    top_assets = ranked[:top_n]
    try:
        return generate_plan(top_assets, weather_forecast=None)
    except Exception:
